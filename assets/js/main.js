...
function handleRiderForm(e){e.preventDefault();const form=e.currentTarget;safeSubmit(form,async()=>{const gate=validateConsentAndHoneypot(form);if(!gate.ok){if(!gate.bot)showFormError(form,gate.message);return}if(!canSubmitNow('entregador')){showFormError(form,'Aguarde alguns segundos antes de enviar novamente.');return}const nome=safeText(form.nome?.value);const whatsapp=safeText(form.whatsapp?.value);const bairro=safeText(form.bairro_cidade?.value);const disponibilidade=form.horarios_disponiveis?.value||'';if(!nome||!whatsapp){showFormError(form,'Preencha nome e WhatsApp para falar com o suporte.');return}if(!isLikelyPhone(whatsapp)){showFormError(form,'Informe um WhatsApp válido com DDD.');return}const msg=`Olá, sou entregador parceiro e quero atendimento da Osasco Express.\n\nNome: ${nome}\nBairro/Cidade: ${bairro||'não informado'}\nDisponibilidade: ${disponibilidade||'não informado'}`;const payload={tipo_formulario:'entregador',origem:'site',nome,whatsapp:normalizePhone(whatsapp),email:'',bairro_cidade:bairro,possui_moto:'',possui_cnh:'',possui_bag:'',dias_disponiveis:'',horarios_disponiveis:disponibilidade,experiencia:'',app_baixado:'Link disponível no site',escala_acessada:'Confirmar horários da semana disponível no site',observacoes:'Formulário simplificado do site',status:'Novo cadastro',pagina:location.href,...getUtm()};const result=await postLeadWithRetry(payload);trackEvent('form_submit',{event_category:'lead',event_label:'entregador_site',page_path:location.pathname,page_title:document.title,intent:'entregador',lead_id:result.lead_id,queued:!!result.queued}); if(window.gtag)gtag('event','generate_lead',{event_category:'lead',event_label:'entregador_site'});showFormSuccess(form,'Mensagem registrada. O suporte abrirá no WhatsApp em instantes.');openWhatsAppWithFallback(form,WA_SUPORTE,msg,'suporte');form.reset()})}
