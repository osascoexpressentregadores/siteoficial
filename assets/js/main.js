...
function handleClientForm(e){e.preventDefault();const form=e.currentTarget;const btn=form.querySelector('button[type="submit"]');const nome=form.nome?.value.trim();const whatsapp=form.whatsapp?.value.trim();const bairro=form.bairro_cidade?.value.trim()||'';const empresa=form.empresa?.value.trim()||'';const cargo=form.cargo?.value.trim()||'';const email=form.email?.value.trim()||'';const tamanhoTime=form.tamanho_time?.value||'';const tipo=form.tipo_operacao?.value||'';const volume=form.volume?.value||'';const problema=form.problema?.value||'';const urgencia=form.urgencia?.value||'';const melhorHorario=form.melhor_horario?.value.trim()||'';const canalPreferido=form.canal_preferido?.value||'WhatsApp';if(!nome||!whatsapp||!tipo||!volume||!problema){showFormError(form,'Preencha nome, WhatsApp, tipo de operação, volume e principal necessidade.');return}clearFormMessages(form);lockButton(btn,'Enviando e abrindo WhatsApp...');const msg=`Olá, vim pelo site da Osasco Express e quero uma orientação inicial.
...
Principal necessidade: ${problema}
Urgência: ${urgencia || 'não informado'}
Melhor horário: ${melhorHorario || 'não informado'}
Canal preferido: ${canalPreferido || 'WhatsApp'}`;const payload={tipo_formulario:'cliente',origem:'site',nome,whatsapp,email,empresa,cargo,tamanho_time:tamanhoTime,tipo_operacao:tipo,nome_negocio:'',volume,periodo_critico:'',modelo_atual:'',problema,urgencia,melhor_horario:melhorHorario,canal_preferido:canalPreferido,bairro_cidade:bairro,frequencia:'',observacoes:'',temperatura:leadTemp(volume,problema),status:'Novo cadastro',pagina:location.href,dispositivo:navigator.userAgent,mensagem_whatsapp:msg,...getUtm()};sendSheet(payload).finally(()=>{trackEvent('form_submit',{event_category:'lead',event_label:'cliente_site',page_path:location.pathname,page_title:document.title,intent:document.body.dataset.intent||'',service:tipo,volume:volume}); if(window.gtag)gtag('event','generate_lead',{event_category:'lead',event_label:'cliente_site'});showFormSuccess(form);const opened=window.open(wa(WA_CLIENTE,msg),'_blank','noopener');if(!opened) location.href=wa(WA_CLIENTE,msg);form.reset();unlockButton(btn)})}
...
(function(){
  ...
  document.addEventListener('focusin', function(e){
    ...
    trackEvent('form_start',{page_path:location.pathname,page_title:document.title,form_type:(form.dataset.form || (form.hasAttribute('data-client-form') ? 'cliente' : 'geral'))});
  });
...
/* Round 6: analytics/performance - extra visibility and external link tracking */
document.addEventListener('click', function(e){
  var a=e.target.closest&&e.target.closest('a[href^="http"]');
  if(!a)return;
  try{ var u=new URL(a.href); if(u.hostname!==location.hostname && u.hostname.indexOf('wa.me')===-1){ trackEvent('external_link_click',{page_path:location.pathname,page_title:document.title,destination:a.href,button_text:(a.textContent||'').trim()}); }}catch(err){}
});
